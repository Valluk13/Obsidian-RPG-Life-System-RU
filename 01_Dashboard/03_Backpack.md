...
(async () => {
    ...
    try {
        ...
        for (let id in inventory) {
            ...
            card.createEl('div', {text: "В наличии: " + qty + " шт.", attr: {style: 'color: #3498db; font-weight: bold; margin-bottom: auto; font-size: 0.95em;'}});
            ...
            useBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    setTimeout(() => { 
                        if (useBtn && document.body.contains(useBtn)) useBtn.innerText = "Использовано"; 
            ...
            sellBtn.addEventListener('click', async () => {
                ...
                try {
                    await app.fileManager.processFrontMatter(tFile, (f) => {
                        f.inventory[id] = Math.max(0, (parseInt(f.inventory[id]) || 0) - 1);
                        f.gold_spent = Math.max(0, (parseInt(f.gold_spent) || 0) - itemInfo.cost);
                    });
                    new Notice("💰 Предмет продан! Вы вернули " + itemInfo.cost + " GP.");
                } finally {
                    setTimeout(() => { 
